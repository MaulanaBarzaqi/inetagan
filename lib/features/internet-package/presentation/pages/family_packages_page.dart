...
class FamilyPackagesPage extends StatefulWidget {
  const FamilyPackagesPage({super.key});

  @override
  State<FamilyPackagesPage> createState() => _FamilyPackagesPageState();
}

class _FamilyPackagesPageState extends State<FamilyPackagesPage> {
  refresh() {
    context.read<FamilyPackageBloc>().add(OnFamilyPackageEvent());
  }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator.adaptive(
      ...
      child: BlocBuilder<FamilyPackageBloc, FamilyPackageState>(
        builder: (context, state) {
          if (state is FamilyPackageLoading) {
            return CircleLoadingWidget();
          }
          if (state is FamilyPackageFailed) {
            return TextFailureWidget(message: state.message);
          }
          if (state is FamilyPackageSuccess) {
